...
if __name__ == '__main__':
    ...
    # cv2.imwrite("dog102.jpg",a)
    # key_leftright,key_updown=cl("dog102.jpg")
    # waterImg = cv2.imread('clsy.png')
    waterImg = cv2.imread('yj.png')
    # waterImg = cv2.resize(waterImg,(102,102))
    Img = cv2.imread('123.png')
    Img = cv2.resize(Img, (400, 400))
    cv2.imwrite("400.png",Img)
    b0, g0, r0 = cv2.split(Img)
    b1, g1, r1 = cv2.split(waterImg)
    b1 = arnold(b1, 2)
    g1 = arnold(g1, 2)
    r1 = arnold(r1, 2)
    b, a0, v0 = setwaterMark1(b1, b0,0,0.17)
    g, a1, v1 = setwaterMark1(g1, g0,1,0.15)
    r, a2, v2 = setwaterMark1(r1, r0,2,0.15)
    get= cv2.merge([b,g,r])
    cv2.imshow('1',get)
    cv2.imwrite("get.png",get)
    # 读取原始图像、嵌入水印图像
    originalImage = cv2.imread('123.png')
    waterImg = cv2.imread("get.png")
    ...
    g1 = g/a1
    r1 = r/a2
    b = getwaterMark1(b0, b1, 2, v0,0,1/0.17)
    g = getwaterMark1(g0, g1, 2, v1,1,1/0.15)
    r= getwaterMark1(r0, r1, 2,v2,2,1/0.15)

    new = cv2.merge([b,g,r])
    cv2.imshow("new",new)
    cv2.imwrite("ok.png",new)
    cv2.waitKey(0)
    # cv2.imwrite("new.png",new)
    # jm(key_leftright,key_updown)
    # a=cv2.imread("ok.png")
    # cv2.imshow("jg",a)
    # cv2.waitKey(0)
